...
import type { JobAndRecruitAmountItem } from '@/typescript/interface/recruitment';
...
const projectDescription = ref(getDefaultEditorTemplate());
...
const requestConfirmPosting = async () => {
  try {
    const confirmResult = confirm('모집글 작성을 완료하시겠습니까?');

    if (!confirmResult) {
      return;
    }

    const confirmLocation = 'create';
    ...
    await useRecruitment().confirmPosting(confirmLocation, title, subject, recruitJobs, skills, content, profitable, meetingType, term, representImage);
  } catch (err) {
  ...
  <div class="flex flex-col w-full fixed bottom-0 bg-backround z-20">
    ...
    <div class="flex flex-col items-center">
      ...
      <div class="pb-6 w-[1156px] md:w-[874px] md:px-6 sm:w-[592px] max-sm:w-[350px]">
        <WpButton @click="requestConfirmPosting" :disabled="confirmButtonDisabled" class="w-full"> 작성 완료 </WpButton>
      </div>
    </div>
  </div>
</template>
